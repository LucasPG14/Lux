...
namespace Amethyst
{
	static GLenum ShaderTypeFromString(const std::string& string)
	{
		...
		AMT_CORE_ASSERT(false, "This shader doesn't exist!");
		return 0;
	}
	...
	void OpenGLShader::CompileShader(const std::unordered_map<GLenum, std::string>& map)
	{
		...
		AMT_CORE_ASSERT(map.size() <= 2, "More than 2 shaders not supported!");
		...
		for (auto& src : map)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				AMT_CORE_ASSERT(false, "Shader Compilation Failed!");
				break;
			}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcessShader(const std::string& src)
	{
		...
		while (pos != std::string::npos)
		{
			...
			AMT_CORE_ASSERT(eol != std::string::npos, "Syntax Error");
			size_t begin = pos + typeLength + 1;
			std::string shader = src.substr(begin, eol - begin);
			
			AMT_CORE_ASSERT(shader == "vertex" || shader == "fragment", "Invalid Shader Type");
	...
	std::string OpenGLShader::ReadFile(const std::string& path)
	{
		...
		else AMT_CORE_ERROR("Couldn't open file '{0}'", path);
